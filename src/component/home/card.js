import { addproduct, corder } from "../../api/orders";
...
function Card(props) {
  ...
  const [orders_id, setOrders_id] = useState(1);
  ...
  const addo = () => {
    ...
    corder(credentials).then((data) => {
      setOrders(data);
    });

    orders.map((order) => setOrders_id(order.id));
    const info = {
    ...
    <section className="product">
      ...
      <div className="product__info">
        <div className="title">
          ...
          <span>COD: {props.product.id}</span>
        </div>
        <div className="price">
          $ <span>{props.product.price}</span>
        </div>
        <div className="offer">
          <span>{props.product.offer}</span>
        </div>
        <div className="description">
          {props.product.description}
        </div>
        <button className="buy--btn" gh={props.product.id} onClick={addo}>
          ADD TO CART
        </button>
        <Counter
